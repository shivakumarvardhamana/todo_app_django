...
Part 1: Running the CSV Server Container
    ...
    Initial Setup:
        The infracloudio/csvserver:latest container was run using the command:

        bash

    docker run -d --name csvserver infracloudio/csvserver:latest

    The container exited with an error (Exited (1)) due to a missing file.

Error Troubleshooting:

    Upon inspecting the logs using docker logs csvserver, the following error was observed:
...
Running the Container with the Input File:
    ...
    The container was run with the following command:
...
Setting Environment Variables:
    ...
    The container was restarted with an environment variable to change the border color:
